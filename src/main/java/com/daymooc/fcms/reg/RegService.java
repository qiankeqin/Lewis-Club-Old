...
public class RegService
{
	...
	public Ret reg(String email, String userName, String password, String nickName, String ip) {
		...
		if (account.save()) 
		{
			...
			{
				return Ret.ok("msg", "注册成功，激活邮件已发送(如未收到，请检查邮箱垃圾箱)，请查收并激活账号：" + userName);
			} else
			{
				return Ret.fail("msg", "注册成功，但是激活邮件发送失败，可能是邮件服务器出现故障，请去Lewis社区官方QQ群留言给群主，多谢！");
			}
		} 
		else 
		{
	...
	private boolean sendRegActivateAuthEmail(String authCode, User reg) {
		String title = "Lewis社区会员激活邮件";
		String content = "欢迎加入Lewis,在这里，你可以分享你的生活。在浏览器地址栏里输入并访问下面激活链接即可完成账户激活：\n\n"
	...
	public Ret activate(String authCodeId) {
		...
		if (authCode != null && authCode.isValidRegActivateAuthCode()) {
			...
			if (n > 0) {
				...
				return Ret.ok("msg", "账号激活成功，欢迎加入Lewis社区！");
			} else {
    ...
    private void sendWelcomeMessage(Integer accountId) {
        try {   // try catch 确保主流程一定成功
            String sysMsg =
                    "您好，我是Lewis的站长 LiuZhaoPo，非常欢迎您的加入。" +
                    "<br/><br/>Lewis社区是一个分享交流平台，" +
                    "欢迎分享你的文章及视频。" +
